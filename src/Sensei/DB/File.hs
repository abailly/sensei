...
readAll = FileDB $ do
  ...
  liftIO $ withBinaryFile file ReadMode (fmap (reverse . snd) . loop appendEventView "" (0, []))
 where
...
readViewsFile UserProfile{userName, userTimezone, userEndOfDay, userProjects} file =
  withBinaryFile
    ...
    ( fmap reverse
        . loop
          (flowViewBuilder userName userTimezone userEndOfDay userProjects)
          userName
          []
    )
...
readNotesFile UserProfile{userName, userTimezone, userProjects} file =
  withBinaryFile
    ...
    ( fmap reverse
        . loop
          (notesViewBuilder userName userTimezone userProjects)
          userName
          []
    )
...
readCommandsFile UserProfile{userName, userTimezone, userProjects} file =
  withBinaryFile
    ...
    ReadMode
    ( fmap reverse
        . loop (commandViewBuilder userTimezone userProjects) userName []
    )

...
mkCommandView ::
    TZLabel -> ProjectsMap -> Event -> Maybe CommandView
mkCommandView (tzByLabel -> tz) projectsMap (EventTrace Trace{..}) =
    Just
        CommandView
            { commandStart = utcToLocalTimeTZ tz _traceTimestamp
...
appendFlow :: TZLabel -> TimeOfDay -> ProjectsMap -> EventView -> [FlowView] -> [FlowView]
appendFlow _ _ _ EventView{event = EventFlow Flow{_flowType = End}} [] = []
appendFlow (tzByLabel -> tz) _ _ EventView{event = EventFlow Flow{_flowType = End, ..}} (v : vs) =
    v{flowEnd = utcToLocalTimeTZ tz _flowTimestamp} : vs
appendFlow (tzByLabel -> tz) dayEnd projectsMap EventView{event = EventFlow Flow{..}} views =
    let view = FlowView st st _flowType (projectsMap `selectProject` _flowDir)
        st = utcToLocalTimeTZ tz _flowTimestamp
     in case views of
...
normalizeViewsForDay startOfDay endOfDay views@(FlowView{flowStart, flowProject} : _) =
    let st' =
            [ FlowView{flowStart = startOfDay, flowEnd = flowStart, viewType = Other, flowProject}

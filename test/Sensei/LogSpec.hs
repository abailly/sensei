...
spec = withApp app $
  describe "Log API" $ do
    ...
    it "GET /logs/<user>?page=2 returns next 50 events and link to previous page" $ do
      events <- liftIO $ generate (sequence $ map (generateEvent (UTCTime (toEnum 50000) 0)) [1 :: Integer .. 100])
      mapM_ postEvent_ events

      getJSON "/log/arnaud?page=2"
        `shouldRespondWith` ResponseMatcher 200 ["Link" <:> encodeUtf8 (writeLinkHeader [ Link "/logs/arnaud?page=1" [(Rel, "prev"), (Other "page", "1")]])] (jsonBodyEquals $ drop 50 $ reverse events)
    ...
    it "GET /logs/<user> returns 1 page and link to next page given there's more events" $ do
      ...
      getJSON "/log/arnaud"
        `shouldRespondWith` ResponseMatcher 200 [ "Link" <:> encodeUtf8 (writeLinkHeader [ Link "/logs/arnaud?page=2" [(Rel, "next"), (Other "page", "2")]])] (jsonBodyEquals $ take 50 $ reverse events)
